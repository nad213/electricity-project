 ...
 */
window.ElecStat = window.ElecStat || {};

ElecStat.PLOT_CONFIG = { responsive: true, displayModeBar: false };
 ...
 */
ElecStat._showOverlay = function(id) {
 ...
 */
ElecStat._hideOverlay = function(id) {
    var el = document.getElementById(id);
    if (!el) return;
    var container = el.parentElement;
    if (!container) return;
 ...
 */
ElecStat.renderChart = function(id, json, config) {
    var el = document.getElementById(id);
    if (!el) return;

    ElecStat._hideOverlay(id);

    try {
        var chart = JSON.parse(json);
        Plotly.newPlot(el, chart.data, chart.layout, config || ElecStat.PLOT_CONFIG);
    } catch (err) {
        _chartError(el);
        if (window.console) console.error('ElecStat.renderChart(' + id + '):', err);
    }
};
 ...
 */
ElecStat.loadCharts = function(url, targetIds) {
    targetIds.forEach(function(id) { ElecStat._showOverlay(id); });
            ...
            fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                ...
                .then(function(data) {
                    ...
                    Object.keys(charts).forEach(function(id) {
                        ...
                        try {
                            var c = charts[id];
                            Plotly.newPlot(el, c.data, c.layout, ElecStat.PLOT_CONFIG);
                        } catch (err) {
                            _chartError(el);
                            if (window.console) console.error('ElecStat.loadCharts(' + id + '):', err);
                        }
                    });
                    ...
                    targetIds.forEach(function(id) { ElecStat._hideOverlay(id); });
                })
                .catch(function(err) {
                    ...
                    if (window.console) console.error('ElecStat.loadCharts:', err);
                });
        });
    });
};
